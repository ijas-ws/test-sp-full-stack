...
export function BackendNodeGraphqlMysql({ stack }) {
  const clientName = "backend";
  const environment = "dev";
  const dbName = `backend_database_${environment}`;
  ...
  const dbURI = `mysql://${username}:${password}@${database.dbInstanceEndpointAddress}/${dbName}`;

  const image = ecs.ContainerImage.fromAsset("backend-node-graphql-mysql/", {
